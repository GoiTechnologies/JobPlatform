...
if($action == 'ajax'){
	...
	$campoempresa = " token, tamano, nombre, correo, telefono, cp, ciudad, pwdempresa, foto, identificacion, comprobante, categoria, descripcion, estado";
	$eWhere= " nombre LIKE '%{$query2}%'";
	$eWhere.=" order by nombre";
	...
	$query2 = mysqli_query($con,"SELECT $campoempresa FROM $tableEmpresa where estado = 2 LIMIT $offset,$per_page");
